...
class HealthCheckHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            logger.error(f"خطأ في معالج GET: {e}")
    
    def do_HEAD(self):
        """معالج HEAD للفحوصات السريعة"""
        try:
            self.send_response(200)
            self.end_headers()
        except Exception as e:
            logger.error(f"خطأ في معالج HEAD: {e}")
    ...
    try:
        ...
        while True:
            try:
                ...
                logger.debug(f"خطأ في معالجة الطلب: {e}")
                continue
    except Exception as e:
        logger.error(f"❌ فشل تشغيل خادم الإشارة: {e}")
...
def main() -> None:
    ...
    server_thread.start()
    logger.info("✅ تم إطلاق خيط خادم الإشارة")
    ...
    try:
        ...
        app.run_polling(poll_interval=1.0, allowed_updates=["message", "callback_query"])
    except Exception as e:
